...
function isInsideCircle(circle, point) {
  ...
  const d = Math.sqrt(pointX + pointY);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const intStart = a > b ? b : a;
  const intEnd = b < a ? a : b;
  return `${start}${intStart}, ${intEnd}${end}`;
}
...
function isCreditCardNumber(ccn) {
  return ccn.toString().split('').reverse().map((el, index) => (index % 2 ? +el * 2 : +el))
